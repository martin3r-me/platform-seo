        ...
        <div class="space-y-6">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="URL suchen..."
                       class="border border-gray-200 rounded-lg px-3 py-2 text-[13px] w-64">
                <select wire:model.live="filterIsOwn" class="border border-gray-200 rounded-lg px-3 py-2 text-[13px]">
                    <option value="">Alle URLs</option>
                    <option value="1">Eigene</option>
                    <option value="0">Wettbewerber</option>
                </select>
                <select wire:model.live="filterStatus" class="border border-gray-200 rounded-lg px-3 py-2 text-[13px]">
                ...
                @if(!empty($selectedUrls))
                    <div class="flex items-center gap-2 ml-auto">
                        <button wire:click="enrichSelected" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-gray-300 bg-white text-gray-700 text-[13px] font-medium hover:bg-gray-50 transition-colors">
                            @svg('heroicon-o-arrow-path', 'w-4 h-4')
                            Enrichen ({{ count($selectedUrls) }})
                        </button>
                        <button wire:click="deleteSelected" wire:confirm="Ausgewählte URLs löschen?" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-red-600 text-white text-[13px] font-medium hover:bg-red-700 transition-colors">
                    ...
                    <thead>
                        <tr class="border-b border-gray-200 text-left">
                            ...
                            <th class="px-4 py-3 cursor-pointer hover:text-gray-700" wire:click="sortBy('url')">
                                URL
                                @if($sortField === 'url') <span class="text-[11px]">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif
                            </th>
                            ...
                            <th class="px-4 py-3 text-right cursor-pointer hover:text-gray-700" wire:click="sortBy('keyword_count')">
                                Keywords
                                @if($sortField === 'keyword_count') <span class="text-[11px]">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif
                            </th>
                            <th class="px-4 py-3 text-right cursor-pointer hover:text-gray-700" wire:click="sortBy('total_search_volume')">
                                SV
                                @if($sortField === 'total_search_volume') <span class="text-[11px]">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif
                            </th>
                            <th class="px-4 py-3 text-right cursor-pointer hover:text-gray-700" wire:click="sortBy('visibility_score')">
                                Sichtbarkeit
                                @if($sortField === 'visibility_score') <span class="text-[11px]">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif
                            </th>
                            <th class="px-4 py-3 text-right cursor-pointer hover:text-gray-700" wire:click="sortBy('backlink_count')">
                                Backlinks
                                @if($sortField === 'backlink_count') <span class="text-[11px]">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif
                            </th>
                            ...
                            <th class="px-4 py-3 text-right cursor-pointer hover:text-gray-700" wire:click="sortBy('last_crawled_at')">
                                Aktualisiert
                                @if($sortField === 'last_crawled_at') <span class="text-[11px]">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif
                            </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @forelse($urls as $url)
                            <tr wire:key="url-{{ $url->id }}" class="hover:bg-blue-50/50 transition-colors">
                                ...
                                <td class="px-4 py-2.5">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @endif
                                        <a href="{{ route('seo.urls.show', $url) }}" wire:navigate class="text-[#166EE1] hover:underline truncate block max-w-xs font-medium">
                                            {{ $url->path ?: '/' }}
                                        </a>
                                    </div>
                                    <div class="text-[11px] text-gray-400">{{ $url->domain }}</div>
                                </td>
                                ...
                                <td class="px-4 py-2.5 text-right text-[11px] text-gray-400">
