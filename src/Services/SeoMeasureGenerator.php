        ...
        foreach ($boardRows as $row) {
            if (! empty($row['conflict'])) {
                $created += $this->upsert($portfolio, [
                    ...
                    'expected_result' => 'Kannibalisierung gelöst — eine Owner-Seite besitzt das Thema, gebündelte Sichtbarkeit.',
            ...
            if (! empty($row['pillar_candidate'])) {
                $created += $this->upsert($portfolio, [
                    ...
                    'expected_result' => 'Zentrale Seite rankt für den Kopf-Begriff und verteilt Autorität auf die Brands.',
        ...
        foreach ($units as $u) {
            ...
            if ($u->verified_at && $u->verified_at->lt(now()->subDays($staleDays))) {
                $created += $this->upsert($portfolio, [
                    ...
                    'expected_result' => 'Frische, verlässliche Antwort — höhere Chance auf Zitat in KI-Antworten.',
            ...
            if ($serp && $serp->present && ! $aiCited) {
                $created += $this->upsert($portfolio, [
                    ...
                    'expected_result' => 'Zitat in KI-Antworten für „'.$label.'" — Präsenz auf beiden Flächen.',
    ...
    public function fromAi(SeoPortfolio $portfolio, array $aiMeasures): int
    {
        ...
        foreach ($aiMeasures as $m) {
            ...
            $created += $this->upsert($portfolio, [
                ...
                'expected_result' => trim((string) ($m['expected'] ?? '')) ?: null,
                'source' => 'ai',
